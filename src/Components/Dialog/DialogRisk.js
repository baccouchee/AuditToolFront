...
const DialogRisk = ({ open, handleClose }) => {
  ...
  const { mutate } = useMutation(
    async formData => {
      const resp = await axios.post(`subStream/${workprogram}`, formData)
      return resp.data
    },
    {
      onSuccess: async data => {
    ...
    <Dialog open={open} onClose={handleClose} fullWidth maxWidth="md">
      <DialogTitle>
        <Box display="flex" alignItems="center">
          <Box flexGrow={1}>Add a new Sub Stream</Box>
          <Box>
